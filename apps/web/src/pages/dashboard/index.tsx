import { RootState } from "@bri/store";
import {
  addTable,
  role,
  searchTable,
  sortTable,
} from "@bri/store/features/table-slice";
import { getProfile } from "@bri/store/features/user-slice";
...
export default function Dashboard() {
  ...
  function onBlur(event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) {
    setState((prevState) => ({...prevState, [event.target.name]: event.target.value}))
  }

  function onEdit(id: number) {
    if(id === isEdit) {
      fetch(`/api/user/updated?user=${id}`, {method: 'POST', body: JSON.stringify(state) }).then(async() => {
            ...
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
              <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  ...
                  {selector.user.role === 1 ? <th scope="col" className="px-6 py-3">
                    Action
                    <a href="#">
                      <svg
                        className="w-3 h-3 ms-1.5 inline-flex"
              ...
              <tbody>
                {(selector.table.archive?.length
                  ...
                  <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" key={key}>
                    ...
                    <td className="px-6 py-4">
                      {isEdit === item.id ? (
                        <select defaultValue={item.role} onBlur={onBlur} name="role" id="countries" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 w-40">
                    ...
                    {selector.user.role === 1 ? <td className="flex items-center px-6 py-4 w-48">
                      <a
                        href="#"
